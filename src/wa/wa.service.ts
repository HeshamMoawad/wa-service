...
export class WaService {
    ...
    async init(phone: string, socket: Socket, name?: string): Promise<string> {
        ...
        this.initPromise = new Promise<string>(async (resolve, reject) => {
            try {
                ...
                reject(new Error(`Failed to initialize WhatsApp client: ${error.message}`));
            }
        });
    ...
    private setupEventHandlers(socket: Socket, resolve: (value: string) => void, reject: (reason?: any) => void) {
        if (!this.client) {
        ...
        this.client.on('disconnected', (reason) => {
            ...
            this.clientInitialized = false;
            this.cleanup();
            socket.emit(Events.LOGGED_OUT, { reason: reason || 'Disconnected from WhatsApp' });
        });
    ...
    async sendMessage(to: string, message: string | MessageMedia, options?: MessageSendOptions) {
        try {
            ...
            if (!this.client || !this.connected || !this.clientInitialized) {
                ...
                throw new Error('WhatsApp client is not ready. Please ensure you have called init() and scanned the QR code.');
            }
            ...
            if (!exist) {
                throw new Error(`Recipient ${recipientJid} is not a registered WhatsApp user`);
            }
            
            // Send the message and return the result
            return await this.client.sendMessage(recipientJid, message, options || {});
        } catch (error) {
